...
let description =
  'Lorem ipsum dolor sit amet consectetur adipisicing elit. Illo voluptatum mollitia vero. Provident maiores voluptas dicta, dolorem ullam architecto at itaque neque, cumque perspiciatis natus enim sequi quam atque aut?';
let products_idsS = [
...
images.forEach((x, i) => {
  ...
  for (let i = 0; i < 3; i++) {
    products_ids.push(
      products_idsS[Math.floor(Math.random() * products_idsS.length)],
    );
  }
...
export const productsDD = productsS;
